...
export const Withdraw = () => {
  ...
  const handleInput = (values) => {    
    let currentBalance = users[userID].balance;
    let withdrawValue = +values.withdraw;
    let updateUsers = users;

    console.log(currentBalance, withdrawValue);

    if (currentBalance < withdrawValue) {
        alert("Transaction Failed");
    }else {

      updateUsers[userID].balance -= withdrawValue;
      setUsers(updateUsers);
      
      //Update Balance Heading value
      withdrawHeading.textContent = "$ " + users[userID].balance.toString();
